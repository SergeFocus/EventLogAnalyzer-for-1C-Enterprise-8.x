...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	Если НЕ РежимФормированияОтчета = "ТолькоФормированиеОтчета" Тогда
		...
		Если ЭтотОбъект.КэшЖурналаРегистрации.Количество() = 0
			...
			Для Каждого Стр Из ТабЖур Цикл
				...
				Стр.МоментВремени = 
					(Стр.Дата - Дата(1,1,1))    // Дата в секундах
					* 1000 					    // Переводим в миллисекунды
					+ НомерОбрабатываемойСтроки;// Добавляем миллисекунду к дате				
	...
	Если НЕ РежимФормированияОтчета = "ТолькоЖурналРегистацииОтчет" Тогда
		...
		Если ЭтотОбъект.ИспользуетсяБСП Тогда
			// TODO Добавить поле для работы с ссылкой на пользователя в базе (справочник Пользователи из БСП)
			//НовоеПоле = СхемаКомпоновкиДанных.НаборыДанных.ДанныеЖурналаРегистрации.Поля.Добавить(Тип("ПолеНабораДанныхСхемыКомпоновкиДанных"));	
			//НовоеПоле.Заголовок = "Пользователь (ссылка)";
			//НовоеПоле.Поле = "ПользовательСсылка";
			//НовоеПоле.ПутьКДанным = "ПользовательСсылка";
			//НовоеПоле.ТипЗначения = Новый ОписаниеТипов("СправочникСсылка.Пользователи");
		КонецЕсли;		
