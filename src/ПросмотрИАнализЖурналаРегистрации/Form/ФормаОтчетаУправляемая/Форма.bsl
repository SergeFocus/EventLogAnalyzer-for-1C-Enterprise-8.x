...
Процедура ПриИзмененииФильтровЖурналаРегистрации(Элемент)
	...
	Если НЕ ТипЗнч(Элемент) = Тип("ПолеФормы")
		И НЕ ТипЗнч(Элемент) = Тип("ТаблицаФормы") Тогда
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьОтборПоМетаданнымИзСтрокиДерева(ПодчиненныеЭлементыДерева, СписокОтборов, Знач ТекущееЗначениеСтрока, Знач ТекущееПредставлениеСтрока)	
	...
	Для Каждого Эл Из ПодчиненныеЭлементыДерева Цикл
		...
		КонецЕсли;
		ДобавитьОтборПоМетаданнымИзСтрокиДерева(ПодчиненныеЭлементыСтрокиДерева, Отчет.События, ТекущееЗначениеСтрокаДерева, ТекущееПредставлениеСтрокаДерева);		
	КонецЦикла;
