...
Процедура ОбработкаНачалаВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ИначеЕсли Элемент.Имя = "Метаданные" Тогда
		...
		ПередаваемыеПараметры.Вставить("ВыбранныеОбъектыМетаданных", Отчет.ФильтрМетаданные);
...
Процедура УстановитьУровеньГруппировок(Команда)
	...
	Попытка
		Уровень = Число(УровеньГруппировкиСтрока);
	Исключение
	КонецПопытки;
...
&НаКлиенте
Процедура ДекорацияИсточникЖурналаРегистрацииРасширеннаяПодсказкаНажатие(Элемент)
	
	Режим = РежимДиалогаВыбораФайла.Открытие;
	ДиалогОткрытияФайла = Новый ДиалогВыбораФайла(Режим);
	ДиалогОткрытияФайла.МножественныйВыбор = Ложь;
	ДиалогОткрытияФайла.Заголовок = "Выберите файл журнала регистрации";
	ДиалогОткрытияФайла.Показать(Новый ОписаниеОповещения("ДекорацияИсточникЖурналаРегистрацииРасширеннаяПодсказкаНажатиеЗавершение", ЭтаФорма, Новый Структура("ДиалогОткрытияФайла", ДиалогОткрытияФайла)));
		
КонецПроцедуры

&НаКлиенте
Процедура ДекорацияИсточникЖурналаРегистрацииРасширеннаяПодсказкаНажатиеЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	
	Если (ВыбранныеФайлы <> Неопределено) Тогда
		
		Отчет.ПутьКФайлуЖурналаРегистрации = ВыбранныеФайлы.Получить(0);
		
		ПриИзмененииФильтровЖурналаРегистрации(Элементы.ПутьКФайлуЖурналаРегистрации);				
...
Процедура ОбновитьЭлементИсточникаДанныхЖурналаРегистрации()
	...
	Если ЗначениеЗаполнено(ПутьКФайлуЖурналаРегистрации) Тогда
		...
		ЭлементФормыИсточника.Картинка = БиблиотекаКартинок.ОткрытьФайл;
		ЭлементФормыИсточника.РасширеннаяПодсказка.Заголовок = "Файл журнала регистрации """+ФайлЖурналарегистрации.Имя+"""";
	Иначе	
		ЭлементФормыИсточника.Картинка = БиблиотекаКартинок.ЖурналРегистрации;
		ЭлементФормыИсточника.РасширеннаяПодсказка.Заголовок = "Журнал этой информационной базы";
	КонецЕсли;
