	...
	Если (МассивЗначений.Количество() - 1) >= Уровень
		...
		Если РезультатПоискаСтрок.Количество() = 0 Тогда
			...
			Если ВыборКонкретныхЭлементов Тогда
				ПолноеЗначениеСобытия = ПолучитьПолноеЗначениеЭлемента(МассивЗначений, Уровень);
...
&НаСервере
Функция ПолучитьПолноеЗначениеЭлемента(МассивЗначений, КонечныйУровень)
...
#Область Служебные_Дополнительно
